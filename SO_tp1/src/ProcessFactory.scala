...
object  ProcessFactory {
  ...
  def buildProcess():Process={
      val r = new Random()
      var newP =  new Process(nextId,Process.READY_STATE, 2*(1 + r.nextInt(30) ),0,0)//gera processos com no maximo 100 quantuns necessarios
      nextId+=1
      newP
  }
  ...
  def buildProcessQueue( numProcess: Int): Queue[Process] ={//constroi uma fila de processos com processos de cpu ,hd e impressora
      ...
      var numOther = if( perc > 0.5 ) Math.ceil((perc-0.5)*numProcess) else Math.ceil(perc*numProcess)  //define q o numero de processos de hd ou impressora seja no maximo 50% do total de processos q o usuario digitou
    ...
    def buildQueues(numProcess: Int): Array[Queue[Process]]={

        var queues =  Array(this.buildProcessQueue(numProcess), new Queue[Process](), new Queue[Process]())
        queues
